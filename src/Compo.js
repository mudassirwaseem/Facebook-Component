...
function Facebook1() {
    const [handle, sethandle] = useState({ name: "", profile: "", img: "" ,text: ""})
    const [posts, setposts] = useState([])

    const Set = () => {
        setposts([...posts, handle])
        sethandle({ name: "", profile: "", img: "" ,text:""})
    }
        ...
        <div>
            ...
            <div style={{ width: "100%", display: "flex", flexDirection: 'column', alignItems: "center", margin: "30px", justifyContent: "center" }}>
                ...
                <div className="form-group" style={{width: "80%"}}>
                    ...
                    <input value={handle.text} onChange={e => sethandle({ ...handle, text: e.target.value })} type="text" className="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter Your Text" />
                </div>
            </div>
